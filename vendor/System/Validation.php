...
class Validation
{
    //This class validates all forms input fields and show errors (if any)
    ...
    public function required($inputName, $customErrorMessage = null)
    {
        ...
        if ($inputValue === '') {// if (!$inputValue)   can not be used because it would consider 0 value as no value
    ...
    public function unique($inputName, array $databaseData, $customErrorMessage = null)
    {
        ...
        if (count($databaseData) == 2) {//The array contains 2 elements: table name, input name
        ...
        if ($exceptionColumn AND $exceptionColumnValue) {//If there are exceptionColumn and exceptionColumnValue
        ...
        if ($result) {//If there are results, then it is NOT unique
